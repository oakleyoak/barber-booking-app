...
-- UPDATED VERSION: Includes auth_user_id, normalized FK references, cleaned duplicates
-- ===================================================================
...
-- Users table (existing) - UPDATED with auth_user_id
...
-- Shop settings table (enhanced) - CLEANED (removed duplicate)
...
-- Daily operations log - FIXED FK to reference public.users
...
-- Cleaning logs - FIXED FK to reference public.users
...
-- Daily cleaning log - FIXED FK to reference public.users
CREATE TABLE IF NOT EXISTS public.daily_cleaning_log (
  id uuid not null default gen_random_uuid (),
  date date not null default CURRENT_DATE,
  barber_id uuid null,
  task_id uuid null,
  ...
  constraint daily_cleaning_log_barber_id_fkey foreign KEY (barber_id) references users (id),
  constraint daily_cleaning_log_task_id_fkey foreign KEY (task_id) references cleaning_tasks (id)
) TABLESPACE pg_default;

-- Maintenance tasks - CLEANED (removed duplicate)
...
-- Maintenance logs - FIXED FK to reference public.users
...
-- Equipment maintenance log - FIXED FK to reference public.users
CREATE TABLE IF NOT EXISTS public.equipment_maintenance_log (
  ...
  constraint equipment_maintenance_log_barber_id_fkey foreign KEY (barber_id) references users (id),
...
-- Safety check logs - FIXED FK to reference public.users
...
-- Daily safety checks - FIXED FK to reference public.users
CREATE TABLE IF NOT EXISTS public.daily_safety_checks (
  id uuid not null default gen_random_uuid (),
  date date not null default CURRENT_DATE,
  barber_id uuid null,
  item_id uuid null,
  ...
  constraint daily_safety_checks_barber_id_fkey foreign KEY (barber_id) references users (id),
...
-- Staff accountability - FIXED FK to reference public.users
...
-- Incident reports - FIXED FK to reference public.users
